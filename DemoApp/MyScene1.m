...
{
    SKSpriteNode *enemy1 =
    [SKSpriteNode spriteNodeWithImageNamed:@"enemy1"];
    enemy1.name = @"enemy1";
    CGPoint enemyScenePos = CGPointMake(
                                        self.size.width + enemy1.size.width/2,
                                        ScalarRandomRange(enemy1.size.height/2,
                                                          self.size.height-enemy1.size.height/2));
    enemy1.position = [self convertPoint:enemyScenePos toNode:_bgLayer];
    [_bgLayer addChild:enemy1];
    
    SKAction *actionMove =
    [SKAction moveByX:-self.size.width + enemy1.size.width y:0 duration:2.0];
...
- (void)update:(NSTimeInterval)currentTime
{
    ...
    if (_lives <= 0 && !_gameOver) {
        ...
        [[GameOverScene alloc] initWithSize:self.size won1:NO];
        // 2
        SKTransition *reveal =
...
- (void)checkCollisions
{
    ...
    [_bgLayer enumerateChildNodesWithName:@"enemy1"
                               usingBlock:^(SKNode *node, BOOL *stop){
                                   SKSpriteNode *enemy1 = (SKSpriteNode *)node;
                                   CGRect smallerFrame = CGRectInset(enemy1.frame, 20, 20);
...
- (void)moveTrain
{
    ...
    if (trainCount >= 6 && !_gameOver) {
        ...
        // 1
        SKScene * gameOverScene =
        [[GameOverScene alloc] initWithSize:self.size won1:YES];
        // 2
        SKTransition *reveal =
        [SKTransition flipHorizontalWithDuration:0.5];
        // 3
