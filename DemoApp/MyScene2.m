...
{
    SKSpriteNode *enemy2 =
    [SKSpriteNode spriteNodeWithImageNamed:@"enemy2"];
    enemy2.name = @"enemy2";
    CGPoint enemyScenePos = CGPointMake(
                                        self.size.width + enemy2.size.width/2,
                                        ScalarRandomRange(enemy2.size.height/2,
                                                          self.size.height-enemy2.size.height/2));
    enemy2.position = [self convertPoint:enemyScenePos toNode:_bgLayer];
    [_bgLayer addChild:enemy2];
    
    SKAction *actionMove =
    [SKAction moveByX:-self.size.width + enemy2.size.width y:0 duration:2.0];
...
- (void)update:(NSTimeInterval)currentTime
{
    ...
    if (_lives <= 0 && !_gameOver) {
        ...
        [[GameOverScene alloc] initWithSize:self.size won2:NO];
        // 2
        SKTransition *reveal =
...
- (void)checkCollisions
{
    ...
    [_bgLayer enumerateChildNodesWithName:@"enemy2"
                               usingBlock:^(SKNode *node, BOOL *stop){
                                   SKSpriteNode *enemy2 = (SKSpriteNode *)node;
                                   CGRect smallerFrame = CGRectInset(enemy2.frame, 20, 20);
...
- (void)moveTrain
{
    ...
    if (trainCount >= 7 && !_gameOver) {
        ...
        // 1
        SKScene * gameOverScene =
        [[GameOverScene alloc] initWithSize:self.size won2:YES];
        // 2
        SKTransition *reveal =
        [SKTransition flipHorizontalWithDuration:0.5];
        // 3
