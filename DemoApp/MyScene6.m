...
//  ZombieConga
//
//  Created by Adrian Phillips on 4/2/14.
...
- (void)update:(NSTimeInterval)currentTime
{
    ...
    if (_lives <= 0 && !_gameOver) {
        ...
        [[GameOverScene alloc] initWithSize:self.size won6:NO];
        // 2
        SKTransition *reveal =
...
- (void)moveTrain
{
    ...
    if (trainCount >= 11 && !_gameOver) {
        ...
        // 1
        SKScene * gameOverScene =
        [[GameOverScene alloc] initWithSize:self.size won6:YES];
        // 2
        SKTransition *reveal =
        [SKTransition flipHorizontalWithDuration:0.5];
        // 3
